...
int main (){
	...
	if (procura (str, c, ind) == -1) 
		printf ("O caractere nao foi encontrado.");	 //utiliza a funcao da linha 4 para saber se foi encontrado ou não o caractere 'c', a partir da posiçã 'ind'
	else 
		printf ("a posicao do caractere '%s'' que voce procura apartir da posicao '%d' e: '%d'.", c, ind, procura(str, c, ind));
}